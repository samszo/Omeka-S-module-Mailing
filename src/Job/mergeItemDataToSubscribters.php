...
namespace Mailing\Job;

use Omeka\Job\AbstractJob;
use Omeka\Stdlib\Message;

class mergeItemDataToSubscribters extends AbstractJob
{
    /**
    ...
    public function perform(): void
    {
        ...
        */
        ...
        $listmonk = $services->get('Mailing\ListmonkService');

        //récupère le workspace pour avoir la liste des documents
        $connect = $listmonk->testConnection();

        if(!$connect){
            $logger->info(new Message('Unable to connect to Listmonk. Please check your configuration.'));
        }
        ...
        foreach (array_chunk($itemIds, self::BULK_LIMIT) as $listItemIds) {
            ...
            foreach ($resources as $resource) {
                ...
                $result = $listmonk->mergeSubscripter([
                    'item' => $resource,
                ]);    
